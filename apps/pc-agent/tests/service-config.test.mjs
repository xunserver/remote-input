...
test("user service configurations preserve paths with spaces and special characters", () => {
  const linux = renderLinuxUnit('/opt/Node Runtime/node', '/home/user/Remote "Input"/main.js');
  assert.match(linux, /ExecStart="\/opt\/Node Runtime\/node" "\/home\/user\/Remote \\"Input\\"\/main\.js"/);
  ...
  const windows = renderWindowsLauncher("C:\\Program Files\\node.exe", "C:\\Users\\O'Brien\\Remote Input\\main.js", "C:\\Temp\\agent.pid");
