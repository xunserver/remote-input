...
> **IMPORTANT:** Always run shadcn CLI commands with `npx shadcn@latest`. Do not substitute `pnpm dlx` based on the project's `packageManager`; the pnpm dlx environment can resolve an incompatible Zod dependency for the shadcn CLI. Continue to use the project's detected package manager for non-shadcn dependency installation.
