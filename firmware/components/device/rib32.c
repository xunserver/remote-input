...
#define RIB32_MAX_BASE32_CHARS (((INPUT_RIB32_CHUNK_BYTES * 8) + 4) / 5)
                                  ...
                                  void *ctx)
{
    ...
    const size_t total_chunks = len == 0 ? 1 : (len + INPUT_RIB32_CHUNK_BYTES - 1) / INPUT_RIB32_CHUNK_BYTES;
    if (total_chunks > UINT16_MAX) {
        return ESP_ERR_INVALID_SIZE;
    }

    for (size_t index = 0; index < total_chunks; ++index) {
        const size_t offset = index * INPUT_RIB32_CHUNK_BYTES;
        const size_t remaining = len > offset ? len - offset : 0;
        const size_t chunk_len = remaining > INPUT_RIB32_CHUNK_BYTES ? INPUT_RIB32_CHUNK_BYTES : remaining;
