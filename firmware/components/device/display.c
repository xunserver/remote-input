...
#define INPUT_DISPLAY_WIDTH_PCT 100
#define INPUT_DISPLAY_LABEL_GAP 16

static const char *TAG = "input_display";
...
static const char *input_state_text(input_state_t state)
{
    switch (state) {
    case INPUT_STATE_IDLE:
        return "Input: Idle";
    case INPUT_STATE_RECEIVING:
        return "Input: Receiving";
    case INPUT_STATE_TYPING:
        return "Input: Typing";
    case INPUT_STATE_DONE:
        return "Input: Done";
...
static void reset_handles(void)
{
    ...
    s_input_state = INPUT_STATE_IDLE;
...
static lv_obj_t *create_label(lv_obj_t *parent, const char *text)
{
    ...
    lv_obj_set_width(label, lv_pct(INPUT_DISPLAY_WIDTH_PCT));
...
esp_err_t input_display_init(const char *version)
{
    ...
    lv_obj_set_style_pad_row(s_root, INPUT_DISPLAY_LABEL_GAP, 0);
