...
#define INPUT_LED_GPIO 38
#define INPUT_LED_COUNT 1
#define INPUT_LED_RMT_RESOLUTION_HZ (10 * 1000 * 1000)
#define INPUT_LED_TASK_STACK_SIZE 2048
#define INPUT_LED_TASK_PRIORITY 3
#define INPUT_LED_BLINK_MS 150

static const char *TAG = "input_led";
...
static input_led_mode_t s_mode = INPUT_LED_WAITING_CONNECTION;
...
static input_led_mode_t derive_mode_locked(void)
{
    if (s_typing) {
        return INPUT_LED_TYPING;
    }
    if (s_connected) {
        return INPUT_LED_CONNECTED_IDLE;
    }
    return INPUT_LED_WAITING_CONNECTION;
}

static input_led_mode_t get_mode(void)
{
    input_led_mode_t mode;
...
static void set_solid_for_mode(input_led_mode_t mode)
{
    switch (mode) {
    case INPUT_LED_WAITING_CONNECTION:
        set_pixel(32, 0, 0);
        break;
    case INPUT_LED_CONNECTED_IDLE:
        set_pixel(0, 32, 0);
        break;
    case INPUT_LED_TYPING:
        set_pixel(0, 0, 48);
        break;
    default:
        set_pixel(0, 0, 0);
...
static void led_task(void *ctx)
{
    ...
    bool blink_on = false;
    input_led_mode_t applied_solid_mode = (input_led_mode_t)-1;

    for (;;) {
        const input_led_mode_t mode = get_mode();

        if (mode == INPUT_LED_TYPING) {
            blink_on = !blink_on;
            if (blink_on) {
                set_pixel(0, 0, 48);
            } else {
            ...
            applied_solid_mode = (input_led_mode_t)-1;
            ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(INPUT_LED_BLINK_MS));
            continue;
        }
...
esp_err_t input_led_init(void)
{
    led_strip_config_t strip_config = {
        .strip_gpio_num = INPUT_LED_GPIO,
        .max_leds = INPUT_LED_COUNT,
    };
    led_strip_rmt_config_t rmt_config = {
        .resolution_hz = INPUT_LED_RMT_RESOLUTION_HZ,
    };
    ...
    s_connected = false;
    s_typing = false;
    s_mode = INPUT_LED_WAITING_CONNECTION;
    s_initialized = true;
    portEXIT_CRITICAL(&s_led_lock);

    BaseType_t created = xTaskCreate(led_task,
                                     "input_led",
                                     INPUT_LED_TASK_STACK_SIZE,
                                     NULL,
                                     INPUT_LED_TASK_PRIORITY,
                                     &s_led_task_handle);
    if (created != pdPASS) {
