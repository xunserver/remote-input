...
static void reset_clients(void)
{
    ...
    for (size_t i = 0; i < CONFIG_INPUT_WS_MAX_CLIENTS; i += 1) {
        s_client_fds[i] = -1;
    }
...
static void notify_error(input_error_t error)
{
...
static esp_err_t recv_frame_payload(httpd_req_t *req, httpd_ws_frame_t *frame, uint8_t *payload)
{
    ...
    return httpd_ws_recv_frame(req, frame, INPUT_DATA_FRAME_MAX_LEN);
}

static void handle_binary_frame(const uint8_t *payload, size_t len)
{
    if (payload == NULL) {
        notify_error(INPUT_ERR_INVALID_COMMAND);
        return;
    }

    const uint8_t type = len > 1 ? payload[1] : 0;

    if (len == INPUT_CONTROL_FRAME_LEN && type == INPUT_CONTROL_CONFIG) {
        input_config_frame_t frame;
        if (!input_parse_config_frame(payload, len, &frame)) {
            notify_error(INPUT_ERR_INVALID_COMMAND);
            return;
        }
    ...
    if (len == INPUT_CONTROL_FRAME_LEN &&
        (type == INPUT_CONTROL_START || type == INPUT_CONTROL_COMMIT)) {
        input_control_frame_t frame;
        if (!input_parse_control_frame(payload, len, &frame)) {
            notify_error(INPUT_ERR_INVALID_COMMAND);
            return;
        }
    ...
    if (len >= INPUT_DATA_FRAME_HEADER_LEN && len <= INPUT_DATA_FRAME_MAX_LEN &&
        type == INPUT_DATA_FRAME) {
        input_data_frame_t frame;
        if (!input_parse_data_frame(payload, len, &frame)) {
            notify_error(INPUT_ERR_INVALID_CHUNK);
            return;
        }
...
static esp_err_t ws_handler(httpd_req_t *req)
{
    ...
    uint8_t payload[INPUT_DATA_FRAME_MAX_LEN];
...
{
    if (strlen(CONFIG_INPUT_WIFI_AP_PASSWORD) < 8) {
    ...
    strlcpy((char *)wifi_config.ap.ssid, CONFIG_INPUT_WIFI_AP_SSID, sizeof(wifi_config.ap.ssid));
    strlcpy((char *)wifi_config.ap.password, CONFIG_INPUT_WIFI_AP_PASSWORD, sizeof(wifi_config.ap.password));
    wifi_config.ap.ssid_len = strlen(CONFIG_INPUT_WIFI_AP_SSID);
    wifi_config.ap.channel = CONFIG_INPUT_WIFI_AP_CHANNEL;
    wifi_config.ap.authmode = WIFI_AUTH_WPA2_PSK;
    wifi_config.ap.max_connection = CONFIG_INPUT_WS_MAX_CLIENTS;

    ESP_RETURN_ON_ERROR(esp_wifi_set_config(WIFI_IF_AP, &wifi_config), TAG, "esp_wifi_set_config failed");
    ESP_RETURN_ON_ERROR(esp_wifi_start(), TAG, "esp_wifi_start failed");
    ESP_LOGI(TAG, "wifi ap started ssid=%s channel=%d", CONFIG_INPUT_WIFI_AP_SSID, CONFIG_INPUT_WIFI_AP_CHANNEL);
    return ESP_OK;
}
...
static esp_err_t init_http_server(void)
{
    ...
    config.max_open_sockets = CONFIG_INPUT_WS_MAX_CLIENTS + 2;
    ...
    ESP_LOGI(TAG, "websocket endpoint ready at ws://192.168.4.1%s", INPUT_WS_URI);
    return ESP_OK;
}

void input_ws_notify_status(void)
{
    int fds[CONFIG_INPUT_WS_MAX_CLIENTS];
    copy_client_fds(fds, CONFIG_INPUT_WS_MAX_CLIENTS);
...
const input_receiver_t input_ws_receiver = {
    .name = "ws",
    .init = input_ws_init,
    .notify_status = input_ws_notify_status,
};
