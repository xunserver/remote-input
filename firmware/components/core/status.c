...
static input_status_t g_status;
...
void input_status_init(void)
{
    ...
    g_status.state = INPUT_STATE_IDLE;
    g_status.last_task_id = 0;
    g_status.last_error = INPUT_ERR_OK;
...
void input_status_encode(uint8_t out[14])
{
    ...
    const input_status_t snapshot = input_status_get();

    out[0] = INPUT_PROTOCOL_VERSION;
