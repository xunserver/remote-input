...
{
    if (data == NULL || out == NULL || len != INPUT_CONTROL_FRAME_LEN) {
        return false;
    }
    ...
    if (data[0] != INPUT_PROTOCOL_VERSION) {
        return false;
    }
    if (type != INPUT_CONTROL_START && type != INPUT_CONTROL_COMMIT) {
        return false;
    }
    if (total_bytes > INPUT_MAX_TEXT_BYTES) {
        return false;
    }
    if (total_chunks == 0) {
        return false;
...
bool input_parse_config_frame(const uint8_t *data, size_t len, input_config_frame_t *out)
{
    ...
    if (data[0] != INPUT_PROTOCOL_VERSION || data[1] != INPUT_CONTROL_CONFIG) {
        return false;
    }
    ...
    for (size_t i = 6; i < INPUT_CONTROL_FRAME_LEN; i += 1) {
        if (data[i] != 0) {
            return false;
        }
    }
...
{
    if (data == NULL || out == NULL || len < INPUT_DATA_FRAME_HEADER_LEN ||
        len > INPUT_DATA_FRAME_HEADER_LEN + INPUT_DATA_PAYLOAD_BYTES) {
        return false;
    }

    const uint16_t total_chunks = read_le16(&data[6]);
    const size_t payload_len = len - INPUT_DATA_FRAME_HEADER_LEN;

    if (data[0] != INPUT_PROTOCOL_VERSION) {
        return false;
    }
    if (data[1] != INPUT_DATA_FRAME) {
        return false;
    }
    if (total_chunks == 0) {
        return false;
    ...
    out->total_chunks = total_chunks;
    out->payload = data + INPUT_DATA_FRAME_HEADER_LEN;
