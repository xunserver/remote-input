...
#define AI_INPUT_BLE_DEVICE_NAME "AI-Input-S3"
#define AI_INPUT_DATA_FRAME_MAX_LEN (AI_INPUT_DATA_FRAME_HEADER_LEN + AI_INPUT_DATA_PAYLOAD_BYTES)

#if !defined(BLE_GAP_EVENT_CONNECT) && defined(BLE_GAP_EVENT_LINK_ESTAB)
#define BLE_GAP_EVENT_CONNECT BLE_GAP_EVENT_LINK_ESTAB
#endif

static const char *TAG = "ai_input_ble";
...
static ai_input_ble_callbacks_t s_callbacks;
...
static void set_parse_error(ai_input_error_t error)
{
    ai_input_status_set(AI_INPUT_STATE_ERROR, 0, error, 0, 0);
                             ...
                             struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    uint8_t buf[AI_INPUT_CONTROL_FRAME_LEN];
    uint16_t len = 0;
    int rc = copy_om_to_buffer(ctxt->om, buf, sizeof(buf), &len);
    if (rc != 0) {
        set_parse_error(AI_INPUT_ERR_INVALID_COMMAND);
        return rc;
    }

    ai_input_control_frame_t frame;
    if (!ai_input_parse_control_frame(buf, len, &frame)) {
        set_parse_error(AI_INPUT_ERR_INVALID_COMMAND);
        return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
    }
                          ...
                          struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    uint8_t buf[AI_INPUT_DATA_FRAME_MAX_LEN];
    uint16_t len = 0;
    int rc = copy_om_to_buffer(ctxt->om, buf, sizeof(buf), &len);
    if (rc != 0) {
        set_parse_error(AI_INPUT_ERR_INVALID_CHUNK);
        return rc;
    }

    ai_input_data_frame_t frame;
    if (!ai_input_parse_data_frame(buf, len, &frame)) {
        set_parse_error(AI_INPUT_ERR_INVALID_CHUNK);
        return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
    }
...
esp_err_t ai_input_ble_init(const ai_input_ble_callbacks_t *callbacks)
{
    ...
    int rc = ble_svc_gap_device_name_set(AI_INPUT_BLE_DEVICE_NAME);
    if (rc != 0) {
