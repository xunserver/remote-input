...
#define REMOTE_INPUT_BLE_DEVICE_NAME "Remote-Input-S3"
...
static const char *TAG = "remote_input_ble";
                             ...
                             struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    uint8_t buf[REMOTE_INPUT_CONTROL_FRAME_LEN];
    uint16_t len = 0;
                          ...
                          struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    uint8_t buf[REMOTE_INPUT_DATA_FRAME_MAX_LEN];
    uint16_t len = 0;
    int rc = copy_om_to_buffer(ctxt->om, buf, sizeof(buf), &len);
    if (rc != 0) {
...
static void start_advertising(void)
{
    ...
    rsp_fields.name = (uint8_t *)REMOTE_INPUT_BLE_DEVICE_NAME;
    rsp_fields.name_len = strlen(REMOTE_INPUT_BLE_DEVICE_NAME);
