...
| Data | `9e7b0003-4f2a-4d3b-9c2a-0d6c9a120001` | Write | SDK -> ESP32 |
...
START/COMMIT Control 帧固定为 12 字节。在 BLE 传输中，START 和 COMMIT 帧写入 Control 特征值；在 WebSocket 传输中，START 和 COMMIT 帧分别作为独立 binary message 发送。
...
| 8 | 0..12 | bytes | `payload` | UTF-8 字节片段 |
...
| 6 | TASK_TOO_LARGE | 文本超过 16 KB，或分片数量超过缓冲能力 |
