...
12. ESP32 通过当前传输层返回 DONE 或 ERROR 状态；该状态只表示 RIB32 HID 输入任务完成或失败，不表示目标网页已完成解码。
13. `decode.html` 在目标设备本地校验分块 CRC32 和整篇 CRC32，通过后显示可复制的原始文本。

这里要严格区分 ESP32 的传输层状态和浏览器本地解码结果：DONE/ERROR 只反映 RIB32 输入任务的成败，不是 `decode.html` 最终校验或显示结果的完成信号。`decode.html` 的分块 CRC32、整篇 CRC32 校验，以及“显示可复制原文”都是目标页面本地行为，它们发生在目标设备浏览器中，不作为 ESP32 判定任务成功的条件。
...
目标设备应通过 Vite dev server 或部署后的静态文件打开 `sdk/decode.html`。部署静态文件时，需要同时提供 `decode.html`、`styles.css` 和 `dist/remote-input-decoder.js`。使用前应把目标设备切换到 US/English 键盘布局，关闭 Caps Lock，并关闭会拦截 ASCII 标点的输入法组合状态，然后把光标放在 RIB32 输入框内。ESP32-S3 不再依赖 Windows `EnableHexNumpad`，也不再使用 `Alt` + Unicode 码点输入；这两种方式都只是历史方案，不属于当前协议。
...
- 把目标设备切换到 US/English 键盘布局，关闭 Caps Lock，并关闭会拦截 ASCII 标点的输入法组合状态。
- 将光标聚焦到 RIB32 输入框。
- 发送一段包含中文、英文和标点的文本，确认 ESP32-S3 逐行输入 RIB32 帧。
- 确认页面只在全部 CRC32 与 UTF-8 校验通过后显示可复制原文。
- 手动破坏一处输入字符，确认页面报告分块、整篇或全局解析错误，而不是显示乱码文本。
