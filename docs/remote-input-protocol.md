...
目标设备应通过 Vite dev server 或部署后的 Vite 页面打开 `sdk/decode.html`，不要直接以 `file://` 或原始静态源文件方式打开当前 HTML，因为页面直接引用了开发源码模块。使用前应把目标设备切换到 US/English 键盘布局，关闭 Caps Lock，并关闭会拦截 ASCII 标点的输入法组合状态，然后把光标放在 RIB32 输入框内。ESP32-S3 不再依赖 Windows `EnableHexNumpad`，也不再使用 `Alt` + Unicode 码点输入；这两种方式都只是历史方案，不属于当前协议。
