...
12. ESP32 通过当前传输层返回 DONE 或 ERROR 状态，表示 RIB32 帧输入任务已完成或失败。

`decode.html` 的分块 CRC32、整篇 CRC32 校验，以及“显示可复制原文”都是目标页面本地行为。它们发生在目标设备浏览器中，不影响 ESP32 返回 DONE/ERROR 的时序，也不作为 ESP32 判定任务成功的条件。
...
`EnableHexNumpad` 和 `Alt` + Unicode 码点输入只是历史方案，不属于当前协议。
